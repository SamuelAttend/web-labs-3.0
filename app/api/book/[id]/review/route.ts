...
export async function GET(req: Request) {
    ...
    const { data, err } = await dbQuery('SELECT user_id AS "id", login, review AS "text" FROM book_user_review, user WHERE book_id LIKE ? AND user.id LIKE user_id', [id])
    switch (err) {
        case 'ECONNREFUSED':
            return NextResponse.json({ error: 'Connection issues' }, { status: 500 })
        case 'EDATANOTFOUND':
            return NextResponse.json({ error: 'Book or reviews are not found' }, { status: 400 })
        default:
            break;
    }
